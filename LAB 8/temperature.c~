...
void main(void)
{
	...
	LCD_Init();
	Timer_Init();
	LCD_StringWrite("Temtur:  xxxx C", 15);
	...
	while(1)					// endless 
	{
		                                     ...
		                                     
		{
			...
			avgVal = avgVal*(1024.0/3300.0); // in mV
			LCD_CmdWrite(0x89);		 //
			voltage[0] = avgVal%10 + 0x30; // add 0x30 to convert the corresponding number to ASCII
			avgVal = avgVal/10;
			voltage[1] = avgVal%10 + 0x30;
			avgVal = avgVal/10;
			voltage[3] = avgVal%10 + 0x30;
			avgVal = avgVal/10;
			voltage[2] = '.';
